...
#include "BleComm.h"  // BLE streaming
...
int32_t AMPShowResult(float *pData, uint32_t DataCount)
{
  ...
  for(int i=0;i<DataCount;i++)
  {
    ...
    //sendMessage(dat);
    printf("Current:%f\n", pData[i]);
      ble_push_current(pData[i]);  //change here from OG code -jose //incorporates bluetooth 
}
  return 0;
}
...
static int32_t AD5940PlatformCfg(void)
{
  ...
  fifo_cfg.FIFOSrc = FIFOSRC_DFT; //SINC2NOTCH; //changes made here used to be fifosrc_dft -jose 
  fifo_cfg.FIFOThresh = 1;    //changes made here   
...
void AD5940AMPStructInit(void)
{
  ...
  pAMPCfg->NumOfData = -1;      /* Never stop until you stop it manually by //changes made here if -1 it will never stop outputting data used to be 25 AppAMPCtrl() function */  
  
  /*TOTAL MEASUREMENTS = NunOfData * FifoThresh
  /* Configure measurement parameters */
  pAMPCfg->AmpODR = 1.5;            /* Time between samples in seconds */
