...
AppAMPCfg_Type AppAMPCfg = 
{
  ...
  .AmpODR = 1.0,     //changes made here was 1.0 before -jose            /* Sample time in seconds. I.e. every 5 seconds make a measurement */
  .NumOfData = -1,
...
/* ADC Configure*/
  ...
  .ADCSinc2Osr = ADCSINC2OSR_22, //changes made here used to be 267-jose 
...
fImpPol_Type RtiaCalValue;  /* Calibration result */
  ...
  lprtia_cal.ADCSinc2Osr = ADCSINC2OSR_22;      //changes made here -jose   /* Use SINC2 data as DFT data source */
  lprtia_cal.DftCfg.DftNum = DFTNUM_2048;    //changes made here -jose      /* Maximum DFT number */
  ...
  lprtia_cal.fFreq = AppAMPCfg.AdcClkFreq/4/22/2048*3; //changes made here -jose /* Sample 3 period of signal, 13.317Hz here. Do not use DC method, because it needs ADC/PGA calibrated firstly(but it's faster) */
...
AD5940Err AppAMPInit(uint32_t *pBuffer, uint32_t BufferSize)
{
      ...
      AppAMPCfg.AMPInited == bFALSE) && AppAMPCfg.ExtRtia == bFALSE)  /* Do calibration on the first initializaion */
  {
	...
	/* Reconfigure FIFO */
  AD5940_FIFOCtrlS(DFTSRC_SINC3, bFALSE);	//changes made here used to be DFTSRC_SINC3 -jose 								/* Disable FIFO firstly */
